...
exports.getAllUserInfo = async (req, res) => {
  console.log("we are in get Login user Details Controller ");
  try {
    // get the user id
    const id = req.user.id;
    const userDetails = await User.findById(id)
...
exports.updateProfileImage = async (req, res) => {
  console.log("we are in update display image controller ");
  try {
...
exports.getEnrolledCourses = async (req, res) => {
  ...
  try {
    ...
    let userDetails = await User.findOne({
      ...
      .exec();
      console.log("sdhmf,djfkjsdf########",userDetails);
    ...
    console.log("user me data ",userDetails);
    if (!userDetails) {
      return res.status(400).json({
        success: false,
        message: `Could not find user with id: ${userDetails}`,
