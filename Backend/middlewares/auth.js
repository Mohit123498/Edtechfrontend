...
exports.auth = async (req, res, next) => {
  console.log("We are auth Middleware ");
  try {
...
exports.isStudent = async (req, res, next) => {
  console.log("Now we have in student middleware ");
  ...
  try {
    ...
    if (userDetails.accountType !== "Student") {
      return res.status(401).json({
        success: false,
        message: "This is the protected route Only student can access it. ",
      });
    }
    next();
  } catch (error) {
...
exports.isInstructor = async (req, res, next) => {
  console.log("Now we have in Instructor middleware ");
  ...
  try {
    if (userDetails.accountType !== "Instructor") {
      return res.status(401).json({
        success: false,
        message: "This is the protected route Only Instructor can access it ",
      });
    }
    next();
  } catch (error) {
...
exports.isAdmin = async (req, res, next) => {
  console.log("Now we have in Admin middleware ");
  ...
  try {
    if (userDetails.accountType !== "Admin") {
      return res.status(401).json({
        success: false,
        message: "This is the protected route, Only admin can access it",
      });
    }
    next();
  } catch (error) {
